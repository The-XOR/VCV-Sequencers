...
KleeWidget::KleeWidget(Klee *module) : SequencerWidget()
{
	...
	{
		// Load switches
		ParamWidget *pwdg = createParam<LevettaR>(Vec(mm2px(k * switch_dstx + 11.229), yncscape(114.071 + nkk_offs, 7.336)), module, Klee::LOAD_BUS + k);
		addParam(pwdg);
		
		pwdg = createParam<LevettaW>(Vec(mm2px(k * switch_dstx + 118.914), yncscape(114.071 + nkk_offs, 7.336)), module, Klee::LOAD_BUS + k + 8);
		addParam(pwdg);
		
		// BUS switches
		pwdg = createParam<Levetta3PosB>(Vec(mm2px(k * switch_dstx + 11.229), yncscape(4.502 + nkk_offs, 7.336)), module, Klee::GROUPBUS + k);
		addParam(pwdg);
	
		pwdg = createParam<Levetta3PosB>(Vec(mm2px(k * switch_dstx + 118.914), yncscape(4.502 + nkk_offs, 7.336)), module, Klee::GROUPBUS + k + 8);
		addParam(pwdg);
	}
